...
{
    private const int NEUTRAL_KEY = 0, JOY_KEY = 1, CALM_KEY = 2, ANGER_KEY = 3, SADNESS_KEY = 4;

    public Holomap mHoloMap;
    
    public double mFeatureEnergy = 0,
        mFeatureSymmetrySpatial = 0,
        mFeatureSymmetrySpread = 0,
        mFeatureSpatialExtent = 0;
     //   mFeatureHeadLeaning = 0;
    ...
    private String mTrainingStatus = "Neutral"; // Variable to print

    /**
     * Method to be invoked by the NetLayer class when being trained.
     * This method increments the emotion to be trained: 'mCurrentEmotionTrain' and logs the emotion to be trained next.
     */
    public void NextEmotion()
    {
        switch (mCurrentEmotionTrain)
        {
            case (NEUTRAL_KEY):
                ...
                break;
            case (JOY_KEY):
                mTrainingStatus = "Calm";
                mCurrentEmotionTrain = CALM_KEY;
                break;
            case (CALM_KEY):
                mTrainingStatus = "Anger";
                mCurrentEmotionTrain = ANGER_KEY;
                break;
            case (ANGER_KEY):
                mTrainingStatus = "Sadness";
                mCurrentEmotionTrain = SADNESS_KEY;
                break;
        }

        Debug.Log("Train Emotion: " + mTrainingStatus);
    }
    ...
    void FixedUpdate()
    {
     ...
     //   calcHeadLeaning(mDeltaTime);
        ...
        if (mCurrentFrame == SLIDING_WINDOW)
        {
            // 2 Seconds reached 
        ...
        //    mFeatureHeadLeaning = Math.Round(mTotalHeadLeaning / 50.0f, 6);
            ...
            if (!NetLayer.trained)
            {
                ...
                Debug.Log("Training: " + mCurrentEmotionTrain);
            }
            else
            {
                if (mTrainingStatus != "Trained")
                {
                    mTrainingStatus = "Trained";
                    Debug.Log("DNN is Trained");
                }

                double[] C =
                {
                    (double) mFeatureEnergy,
               ...
               //    (double) mFeatureHeadLeaning
                ...
                mHoloMap.UpdateCurrentEmotion(emotion);
            }
...
/*  void calcHeadLeaning(float deltaTime)
    ...
    void CalcSISpatial()
    {
        ...
        float vertSI = CalcSISpatialAux(mesh.vertices[2].y, mesh.vertices[0].y, mesh.vertices[1].y);
    ...
    void CalcEFCurvature()
    {
        ...
        {
            leftCurvature +=
                CalcCurvatureAux(mLeftHandPositions[i - 1], mLeftHandPositions[i], mLeftHandPositions[i + 1]);
        }
        ...
        {
            rightCurvature += CalcCurvatureAux(mRightHandPositions[i - 1], mRightHandPositions[i],
                mRightHandPositions[i + 1]);
        }
    ...
    {
        float eTotal =
            (HAND_MASS * Mathf.Pow(auxLimbVelocity(mLeftHandObj.transform.position, mLeftHandObjLastPos, deltaTime),
                 2) +
             HAND_MASS * Mathf.Pow(auxLimbVelocity(mRightHandObj.transform.position, mRightHandObjLastPos, deltaTime),
                 2))
            / 2;
    ...
    /**
    * Auxiliar method to the calculation of Energy.
     * 
    ...
    */
    ...
    {
        bool isAllBoundingComponentsVisible = mLeftHandObj.activeInHierarchy && mRightHandObj.activeInHierarchy &&
                                              mHeadObj.activeInHierarchy;
